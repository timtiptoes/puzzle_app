...
instructions_map={"simple_addition": "Add the two numbers to find the letter code",
                  "add_negatives":"Add the numbers to find the letter code",
                 "multiplication_then_addition": "Solve for the letter code",
                 "fraction_addition": "Use the numerator of the improper fraction to find the letter code",
                 "simple_division_problem":"Solve for the letter code",
                 "simplify_ratio":"Use the numerator of the simplified ratio to find the letter code",
                 "two_digit_subtraction":"Use the difference to find the letter code",
                 "two_digit_multiplication":"Use the inner two digits of the product to find letter code",
                 "add_coins":"Find the total value of the coins to find letter code",
                 "exponents_problem":"Solve each to find the letter code",
                 "roots_problem": "Solve each to find letter code",
                 "simple_algebra":"Solve for x to find the letter code",
                 "single_decimal_addition":"Add to find the letter code",
                 "quadratic_equations":"Add the roots to find the letter code",
                 "determinant":"Find the determinant of each matrix",
                 "unit_conversion":"Round each conversion to the nearest integer to find the letter code",
                 "simple_series":"Find the next number in the series to find the letter code",
                 "convert_base":"Convert each to base 10 to find letter code",
                 "linear_system":"Add x+y and find letter code",
                 "simplify_exponents":"Simplify exponents to find letter code",
                 "find_slope":"Use the slope of the line through two points to find letter code",
		 "percent_increase":"Find the closest integer to match the letter code",
                 "compound_interest":"Use the closest amount in thousands below to find letter code",
		 "simple_scientific":"Convert each number to standard notation and find the letter code",
                 "intermediate_scientific":"Convert each number to standard notation and find the letter code",
                 "harder_scientific":"Convert each number to standard notation and find the letter code"}
